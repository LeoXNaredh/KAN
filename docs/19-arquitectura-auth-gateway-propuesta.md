...
> **Estado (2026-08-07): incrementos 1, 2 y 3 de 5 completos.** El Gateway (`apps/gateway`) verifica un JWT de Supabase si llega en `X-User-Token`, reutilizando `AuthPort.getCurrentUser()` (mismo mecanismo de ADR-029) sobre el cliente `service_role` que ya existía (ADR-026) — sin credenciales nuevas (incremento 1). `apps/web`/`apps/mobile` ya mandan ese token (incremento 2). **Incremento 3 (pairing):** `apps/desktop` (Edge Agent) ya puede vincularse a un usuario — un código de un solo uso generado desde `/dispositivos` en `apps/web`, reclamado por el Edge Agent contra `POST /v1/pairing/claim` (ruta sin el token interno, propia, con su propio rate limit), que le devuelve un secreto de larga vida guardado hasheado en `edge_agent_pairings` (`supabase/migrations/0008`). El secreto viaja en el `hello` (`pairingToken`, campo opcional) y el Gateway lo resuelve a un `ownerId` en `WsConnectionManager.onHello()` — ya queda en `AgentRecord.ownerId`, pero **nada lo usa todavía para autorizar** (eso es el incremento 4, el último). Un `pairingToken` que no resuelve no rechaza la conexión — degradación consciente. `apps/desktop` se reinicia tras vincularse (no reconecta en caliente).
