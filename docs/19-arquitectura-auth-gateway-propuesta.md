...
> **Estado (2026-08-07): incrementos 1-4 de 5 completos.** El Gateway (`apps/gateway`) verifica un JWT de Supabase si llega en `X-User-Token` (incremento 1); `apps/web`/`apps/mobile` ya lo mandan (incremento 2); `apps/desktop` puede vincularse a un usuario vía un código de un solo uso, resuelto a un `ownerId` en `AgentRecord` (incremento 3, pairing). **Incremento 4 (autorización real):** `Gateway.executeTool()` rechaza si la capability resuelta pertenece a un Edge Agent vinculado a otro usuario — con `{ success: false, error: "No autorizado..." }`, auditado como `tool.execute.denied`, sin llegar a tocar el dispositivo físico. `AgentRegistry.list()`/`GlobalCapabilityRegistry.list()` (y por lo tanto `GET /v1/agents`/`GET /v1/tools`) filtran del mismo modo: un agente sin vincular sigue abierto para cualquiera; uno vinculado, solo visible/usable por su dueño. **Decisión de diseño:** `TaskOrchestrator.submit()` no lleva este chequeo — los jobs programados lo llaman directo, sin request HTTP, nunca van a tener un `requestingUserId`; el chequeo vive un nivel arriba, en `Gateway.executeTool()`, que es la única puerta real de entrada desde HTTP (cubre tanto la API directa como el flujo de chat, que ya manda `X-User-Token`). Falta solo el incremento 5 (auditoría por usuario) para cerrar P2 del todo.
