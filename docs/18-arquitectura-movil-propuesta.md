...
> **Estado (2026-08-07): propuesta aprobada, incremento 2 de 6 completo.** ADR-029/030/031 oficializados en `docs/00-analisis-y-decisiones.md` e implementados. `apps/mobile` tiene scaffold (Expo SDK 57, NativeWind) + `expo-router` (retrofit, no estaba en el scaffold inicial — ver nota abajo) + pantallas de login/registro conectadas a los casos de uso de `@kan/core`, con rutas protegidas vía `Stack.Protected` según el estado de sesión. Verificado con `expo export` real, sin dispositivo/simulador (no disponible en este entorno) — falta probarlo interactivo contra un proyecto de Supabase real. El resto de este documento sigue siendo la referencia de diseño — cada punto tiene problema/propuesta/costo/prioridad, mismo formato que `docs/16`.
>
