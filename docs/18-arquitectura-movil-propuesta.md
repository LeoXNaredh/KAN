...
> **Estado (2026-08-07): propuesta aprobada, los 6 incrementos completos.** ADR-029/030/031/032 oficializados en `docs/00-analisis-y-decisiones.md` e implementados. `apps/mobile` tiene scaffold + `expo-router` + login/registro (incremento 2) + chat de solo texto con streaming (incremento 3) + voz (incremento 4, ver ADR-032) + imagen adjunta (incremento 5) + **paridad de Dashboard y Automatizaciones** (incremento 6: `/api/status` — mismo hook de polling de 15s que web — y `/api/jobs`/`/api/tools`, ambos ya proxies del Gateway sin cambios de backend). Navegación por pestañas nativas (`Tabs` de `expo-router`: Chat/Dashboard/Automatización) en vez del nav lateral de escritorio. Cero cambios de backend en ningún incremento de esta propuesta. Verificado con `expo export` real (bundle exitoso, 1736 módulos) y typecheck/lint limpios en todo el monorepo; sin dispositivo/simulador en este entorno — falta la prueba interactiva real de cada flujo contra un dispositivo/SDK real y un Gateway/Supabase reales.
>
...
6. ✅ Paridad de Dashboard (`/api/status`) y Automatizaciones (`/api/jobs`/`/api/tools`) — completo. Los 6 incrementos originalmente listados están cerrados.

**Pendiente en todos los incrementos por igual, no específico de ninguno:** ninguna de las pruebas interactivas reales (login contra Supabase real, streaming del chat, grabación/transcripción/síntesis de voz, selección de imagen, creación/cancelación de jobs, estado real del Dashboard) se pudo correr en este entorno de desarrollo — no hay simulador ni dispositivo disponible. Toda la verificación hecha hasta ahora es tipos, lint, y `expo export` (bundle exitoso). La primera prueba real en un dispositivo/simulador queda pendiente de que el usuario la corra.

## 6.1. Incremento 6 — decisiones de implementación

**Navegación.** `apps/mobile/app/(app)/_layout.tsx` pasó de un `Stack` sin configurar (una sola pantalla, el chat) a `Tabs` de `expo-router` con 3 pestañas — Chat, Dashboard, Automatización — mismas 3 secciones que la barra de navegación de `apps/web`'s shell, como pestañas nativas en vez de un nav lateral (patrón mobile-idiomático, no una réplica pixel a pixel de un layout pensado para escritorio).

**Dashboard.** Mismo `/api/status` que `apps/web`'s `DashboardClient` (confirmado sin autenticación — no lee cookies ni `Authorization`), mismo hook de polling cada 15s. Muestra el mismo contenido informativo (estado de Gateway/Edge Agent/IA, capacidades detectadas, dispositivos activos, plugins cargados, actividad reciente) en una columna nativa en vez del grid de tarjetas de escritorio. Deliberadamente sin la grilla `DEVICE_CATALOG` (iconos por tipo de dispositivo) de web — es una capa cosmética sobre los mismos `edgeAgents[].devices`, ya visibles en la sección de resumen; no aporta información nueva, solo una presentación distinta que no vale la pena replicar pixel a pixel.

**Automatizaciones.** Mismo `/api/jobs` (GET/POST) y `/api/jobs/:id` (DELETE) — proxies finos del Gateway, confirmado sin autenticación — y `/api/tools` para el catálogo de capabilities. Dos simplificaciones nativas deliberadas, ninguna a nivel de ADR (son decisiones de UI, no de arquitectura/seguridad, mismo criterio que ya se aplicó sin ADR al elegir texto en vez de iconos para `VoiceButton`):
- **Picker de capability:** RN no tiene un `<select>` nativo. En vez de sumar una librería de picker, es una lista desplegable propia chica (tap para expandir, tap en una opción para elegir).
- **Selector de fecha para jobs "una vez":** sin un date-picker nativo (`@react-native-community/datetimepicker` no está instalado, y no se puede probar su compatibilidad con el SDK real en este entorno de desarrollo) — un campo de texto ISO 8601 más dos botones de atajo ("+1 hora"/"+1 día") que completan el campo automáticamente.

Ningún cambio a `packages/gateway-core` ni a los tipos que expone — `apps/mobile/lib/jobs/types.ts` es un espejo escrito a mano de `apps/web/lib/jobs/types.ts` (no un import de `@kan/gateway-core`, que tiene dependencias de Node como `node-cron` pensadas para el proceso del Gateway — mismo criterio que ya evitó que `apps/mobile` importara `@kan/ai-abstraction`, §6).
