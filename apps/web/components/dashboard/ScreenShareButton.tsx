...
export function ScreenShareButton({ sharing, onClick }: { sharing: boolean; onClick: () => void }) {
  ...
  return (
    <button
      ...
      aria-label={label}
      title={label}
      className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border transition-colors duration-fast focus-visible:outline focus-visible:outline-2 focus-visible:outline-accent ${
        sharing
